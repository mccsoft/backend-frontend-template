...
Usage:
    ...
    -r | --response             Expected HTTP response body
...
wait_for()
{
    if [[ $TIMEOUT -gt 0 ]]; then
        ...
        if [[ $EXPECTEDRESPONSE != "" ]]; then
          echoerr "   expecting response $EXPECTEDRESPONSE"
        fi
    else
    ...
    do
        ...
        if [[ $OUTCOME -eq 0 ]]; then
            if [[ "$EXPECTEDRESPONSE" != "" ]]; then
              RESPONSE=$(curl --connect-timeout 2 --insecure -s $URI)
...
wait_for_wrapper()
{
    ...
    if [[ $QUIET -eq 1 ]]; then
        timeout $BUSY_BOX_TIMEFLAG $TIMEOUT $0 $URI --quiet --child --timeout=$TIMEOUT --code=$EXPECTEDCODE --response=$EXPECTEDRESPONSE &
    else
        timeout $BUSY_BOX_TIMEFLAG $TIMEOUT $0 $URI --child --timeout=$TIMEOUT --code=$EXPECTEDCODE --response=$EXPECTEDRESPONSE &
    fi
    SUBPROCESS_PID=$!
...
do
    case "$1" in
        ...
        -r)
        EXPECTEDRESPONSE="$2"
        if [[ $EXPECTEDRESPONSE == "" ]]; then break; fi
        shift 2
        ;;
        --response=*)
        EXPECTEDRESPONSE="${1#*=}"
